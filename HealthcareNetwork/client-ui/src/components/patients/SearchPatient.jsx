...
const SearchPatient = () => {
    ...
    return (
        <div className="flex flex-col min-h-[calc(100vh_-_150px)] pt-24 items-center">
            {!showDetail ?
                <>
                    <h1 className="text-3xl font-medium my-4">Search Patient</h1>
                    <form onSubmit={setSearchValue}>
                        <div className="flex mb-4 px-4">
                            <input
                                type="text"
                                defaultValue={searchValue}
                                className="w-full text-slate-700 outline-none focus:bg-blue-100 border-gray-400 border-2 px-4 py-2 rounded-l-full"
                                placeholder="Enter Patient name"
                                onChange={(e) => setSearchValue(e.target.value)}
                            />
                            <button
                                className={`bg-blue-500 text-white font-bold py-2 px-4 rounded-r-full ml-2 transition-all duration-200 ${loading ? 'opacity-50 cursor-wait' : 'hover:bg-blue-600'
                                    }`}
