...
const AddPatient = () => {
  ...
  return (
    <>
      <div className="flex flex-col min-h-[calc(100vh_-_150px)] pt-28 items-center">
        <h1 className="text-3xl font-medium my-4 ">{isEdit ? 'Modify Patient data' : 'Add Patient'}</h1>
        <form onSubmit={handleSubmit} className="w-full pr-2 sm:pr-0 pt-10 max-w-lg">
          <div className='grid md:flex px-2 sm:px-0 sm:-mx-3 mb-4 sm:mb-5'>
            <div className={`${'w-full pb-4 sm:pb-0 sm:w-1/2'} ${isEdit ? 'pointer-events-none' : ''}`}>
              <input
                ...
                className="w-full rounded-md border border-[#e0e0e0] bg-white py-2 px-4 font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md"
              />
            </div>
          ...
          <div className='grid md:flex px-2 sm:px-0 sm:-mx-3 pb-4 sm:pb-5'>
            <div className={`${'w-full pb-4 sm:pb-0 sm:w-1/2'} ${isEdit ? 'pointer-events-none' : ''}`}>
              <DatePicker
                ...
                className='block w-full text-gray-700 border rounded py-2 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500'
              />
            </div>
            <div className="w-full sm:pl-3 sm:w-1/2">
              <select
                className="block w-full text-gray-700 border border-gray-200 rounded py-2 px-4  
          ...
          <div className="grid md:flex px-2 sm:px-0 sm:-mx-3 mb-5">
            <div className="w-full pb-4 sm:pb-0 sm:w-1/2">
              <input
                className="appearance-none block w-full text-gray-700 border outline-none border-gray-200 rounded py-2 px-4 leading"
                id="mobile"
                name="mobile"
                type="text"
          ...
          <div className="flex items-center px-2 sm:-mx-3 sm:px-0 justify-between">
            <button
              className={`w-[45%] bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none 
              focus:shadow-outline ${isLoading ? 'spin pointer-events-none' : ''}`}
              type="submit">
              {isLoading ? 'Please wait...' : 'Submit'}
            </button>

            <button
              className="w-[45%] bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
