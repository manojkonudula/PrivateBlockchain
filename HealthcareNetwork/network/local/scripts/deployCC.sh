...
approveForMyOrg() {
  ...
  ORG=$2
  setGlobals $PEER $ORG
  echo "SEQUENCE: lifecycle chaincode approveformyorg -o localhost:1100 --ordererTLSHostnameOverride orderer1 --tls --cafile $ORDERER1_CA --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG}"
...
if [ "$CC_SKIP_PACKAGE_INSTALL" = "true" ]; then
  ...
  # peer0 of hospital
  installChaincode 0 hospital
  # peer1 of hospital
  installChaincode 1 hospital
  # peer0 of pharmacy
  installChaincode 0 pharmacy
  # peer1 of pharmacy
  installChaincode 1 pharmacy
  # peer0 of laboratory
  installChaincode 0 laboratory
  # peer1 of laboratory
  installChaincode 1 laboratory
fi
...
if [ "$CHANNEL_NAME" = "hcarechannel" ]; then
  ...
  queryInstalled 0 hospital

 ## approve the definition for config.orgs
  approveForMyOrg 0 hospital
  approveForMyOrg 0 pharmacy
  approveForMyOrg 0 laboratory
  ## check whether the chaincode definition is ready to be committed
  ## expect all of them to have approved

  checkCommitReadiness 0 hospital
  checkCommitReadiness 0 pharmacy
  checkCommitReadiness 0 laboratory

  ## commit the definition
  commitChaincodeDefinition hospital pharmacy laboratory 

  ## query to see that the definition committed successfully
  queryCommitted 0 hospital
  queryCommitted 0 pharmacy
  queryCommitted 0 laboratory
