...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    if [ "$OS_ARCH" == "darwin-amd64" ]; then
      ...
      rm -rf organizations/fabric-ca/hospital
      rm -rf organizations/fabric-ca/pharmacy
      rm -rf organizations/fabric-ca/laboratory
      ...
      ## remove fabric ca artifacts
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/hospital'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/pharmacy'
      docker run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/laboratory'
